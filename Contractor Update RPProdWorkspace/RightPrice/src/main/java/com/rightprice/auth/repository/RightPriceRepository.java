...
//import com.rightprice.auth.model.DealCreationDetails;
...
public interface RightPriceRepository {
	...
	ResponseEntity<Object> getApproverDesignation(int rcId);
	
	ResponseEntity<Object>getCustomerByVerticalGroupId();
	
	ResponseEntity<Object>getCustomerForUser();

	//ResponseEntity<Object> getApproverName(int verticalId);

	ResponseEntity<Object> getLeaderApproverName();

	ResponseEntity<Object> getDealDetails(String crmDealId, int rpVrsId);
	
	ResponseEntity<Object> updateApprovalDetails(RateCardDetails rateCardApproval);

	ResponseEntity<Object> getCountry(); //sneha
	...
	/*ResponseEntity<Object> updateCityDetails(ViewCitiesData cityDetailsAdd);*/ //sneha

	ResponseEntity<Object> updateCityDetails(ViewCitiesData cityDetailsUpdate);
...
//	ResponseEntity<Object> addRate(MasterRate masterRate);
	...
	ResponseEntity<Object> getVersionDetails(String rpDealId);
	ResponseEntity<Object> getVersionDetailsTM(String rpDealId);
	ResponseEntity<Object> getRiskAnswers(int rpDealVersionId);

/*	ResponseEntity<Object> insertDealData(Deal dealData);*/
	ResponseEntity<Object>insertFpDealData(FpDeal dealData);
	ResponseEntity<Object> insertDealData(Deal dealData);
	int insertDlCurrUTI(DealCurrencyUtilization dcuti);
	int insertDlLoc(DealLocation dlLoc);
	int addRiskAnswers(RiskBasedAnswers answerData);
	int addDevMainAnswers(ProjectTypeAnswers answerDevMainData);
	int updateFpDealData(FpDeal dealData);
	int updateDealData(Deal dealData);

	int updateRiskAnswers(List<RiskBasedAnswers> answersArrray);
	int updateDevMainAnswers(List<ProjectTypeAnswers> answersDevMainArrray);
	...
	ResponseEntity<Object> getVerticalDetails(int rcId);
	...
	/*ResponseEntity<Object> addCityDetails(VisaLabels visaDetailsAdd);*/
	...
	ResponseEntity<Object> getVisaLabel(int countryId, int visaId);
	/*-----------------Visa Screen Sneha*/
	...
	ResponseEntity<Object> saveVerticalMember(VerticalMembers[] verticalMember);
...
*/
	ResponseEntity<Object> getRateCardData(MyDashboardRC[] rateCardCount);
	...
	int addObject(FileObjectTable fileObject);

	ResponseEntity<Object> getCommonCostExportDetails(int countryId, int year);
...
//	ResponseEntity<Object> getTaxAssumptionReportExcel(int country, int deductionId);
	...
	ResponseEntity<Object> getFpDealStaffingData(int rpDealVersionId, int cityId, int towerId);
	ResponseEntity<Object> getFpDealStaffingDataExcel(int rpDealVersionId, int towerId);
	...
	ResponseEntity<Object> uploadCountry(ArrayList<Country> countryBean);
	...
	ResponseEntity<Object> uploadCity(ArrayList<City_STG> cityBean);

	ResponseEntity<Object> getOnsiteFaclityCost(int countryId, int cityId, int towerId);
	...
	ResponseEntity<Object> updateRP_Deal_Table(int rpVrsId, Double calc_total_GM, Double tCVModelValue);
	...
	ResponseEntity<Object> saveCalculateMarginData(RateCardManualMarginCal[] rateCardMarginData);
	...
	ResponseEntity<Object> uploadManualRateCard(MultipartFile file, String fileName, InputStream uploadedInputStream, FileObjectTable fileObj,FPManualDealAttachment fpData, int countryId);
	...
	ResponseEntity<Object> uploadManualRateCardFile(MultipartFile file, String fileName,
			InputStream uploadedInputStream, FileObjectTable fileObj, int rcId);
	...
	//ResponseEntity<Object> getDealApproverName(int verticalId);
...
//	ResponseEntity<Object> getDataOnSearch();
	...
	ResponseEntity<Object> changeActiveStatus(int docId);
	
	ResponseEntity<Object> uploadManualFile(MultipartFile file, String fileName, InputStream uploadedInputStream,
			FileObjectTable fileObj, Integer versionId, FPDealAttachment fpData);
	...
	void getDeliveryListForMailTrigger();
	
	
	ResponseEntity<Object> uploadManualDealFile(MultipartFile file, String fileName, InputStream uploadedInputStream,
			FileObjectTable fileObj, Integer rpDealVersionId,  FPManualDealAttachment fpData, Integer noOfTowers);
	
	
	ResponseEntity<Object> getVersionAttachment(int rpDealVersionId);


	ResponseEntity<Object> getFpDealCostInputsDetails(int towerId, int costType, int rpdealVersionId);
	...
	ResponseEntity<Object> uploadRCManualFile(MultipartFile file, String fileName, InputStream uploadedInputStream,
			FileObjectTable fileObj, Integer rcId, Integer cityId, FPManualDealAttachment fpData);

	
	ResponseEntity<Object> getFPManualDealApproverInfoDesignation(int rpDealVersionId);

	ResponseEntity<Object> updateFPMDApprovalDetails(FpDeal fpmdApproval);

//	ResponseEntity<Object> getFPMDApproverNames(int verticalId, int rpDealVersionId);
	...
	ResponseEntity<Object> getCostBreakupData(int dealVersionId,String cRMDealId);
...
//	ResponseEntity<Object> getDealDataOnSearch(int dealVresionID);
	...
	ResponseEntity<Object> deleteDealVersion(int dltDealVersionId, String string);
	...
	void insertInSkillForecastStaffing();
	...
	ResponseEntity<Object> uploadfpDealStaffingFile(MultipartFile file, String fileName,
			InputStream uploadedInputStream, FileObjectTable fileObj, Integer rpDealVersionId,
			FPManualDealAttachment fpData);

	ResponseEntity<Object> uploadEstimationDealFile(MultipartFile file, String fileName, InputStream uploadedInputStream,
			FileObjectTable fileObj, Integer versionId, FPDealAttachment fpData,EstimationAttachment eAData);
	...
	int getDeligateUserCount(String userName);
	...
	ResponseEntity<Object> uploadManualRateCard_GFT(MultipartFile file, String fileName, InputStream uploadedInputStream, 
			FileObjectTable fileObj,FPManualDealAttachment fpData, int countryId);
	...
	ResponseEntity<Object> getActiveRCData(String date);
	...
	ResponseEntity<Object> getFileredAtosRcData(int country, String rcStartDate,
			String rcEndDate, int currentApStatus);
	...
	void updatedverticalMailTrigger();
	...
	Integer getSubmittedData(String userID);

	ResponseEntity<Object> setSubmitEnabled(String currentUser);

	ResponseEntity<Object> setSubmitDisabled();
/*	
	ResponseEntity<Object> getPreSalesApprovedData(int salesId);

	ResponseEntity<Object> getPreSalesApprovalData(int status);*/

	void CRMHistoryChangesEmailSchedule();
...
/*	ResponseEntity<Object> getOpportunityDetails();
	...
	ResponseEntity<Object> getSyntelUserId(String userName);

	String getSyntelId(String userName);
	...
	ResponseEntity<Object> uploadMasterFile(MultipartFile file, String fileName, InputStream uploadedInputStream,
			FileObjectTable fileObj, FPManualDealAttachment fpData);
	...
	List<String> getUserRolesBasedonDasId(String dasID);

	ResponseEntity<Object> getRCRoleUtilizationAttachement(AttachmentMapper rpDealVersionId);

	ResponseEntity<Object> getFpDealCostInputsDetailsUpdate(int towerId, int rpdealVersionId);
...
*/
	...
	ResponseEntity<Object> CheckUserAccess(Integer accId, String userId);
	...
	fileName, InputStream uploadedInputStream, FileObjectTable fileObj, Integer
	versionId, RateCardAttachment rcData);
	...
	/* Deal*/
	 ResponseEntity<Object> getDealDetailss(String crmDealId);
		...
		ResponseEntity<Object> uploadDealsFile(MultipartFile file, String fileName, DealAttachmentV2 dealdata,
	...
	//	ResponseEntity<Object> getCurrentCustomerUserDetails(int verticalId);
      ...
      //manglam updated
		...
		ResponseEntity<Object> uploadPLFileData(MultipartFile file, String fileName, InputStream uploadedInputStream, FileObjectTable fileObj, Integer versionId, PLAttachment fpData);
		ResponseEntity<Object> getPLAttachementData(AttachmentMapperPL rpDealVersionId);

		//RBU Changes!
		
		ResponseEntity<Object> getApproverName(int verticalId,int rbutype);
