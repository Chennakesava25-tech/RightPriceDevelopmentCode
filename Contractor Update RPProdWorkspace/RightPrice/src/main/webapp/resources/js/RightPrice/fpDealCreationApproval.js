...
app.controller("FPDeal", ['$scope','$location','$anchorScroll','$http','$window','$sessionStorage','WebServiceFactory','$filter', function($scope,$location,$anchorScroll,$http,$window,$sessionStorage,WebServiceFactory,$filter, $index){
			...
			$scope.Next = function() {
			 ...
			 var rpVrsId = $sessionStorage.rpDealVersionId;	
				...
				var getVersionDetails = function(response)  //service will be call from this line
					...
					WebServiceFactory.getVersionDetails($sessionStorage.DealModel).then(getVersionDetails);
		...
		angular.forEach($scope.approverLevels, function(value, key) {
				...
				if( $scope.rateCardInfo[0].currentStatus != "Draft") {
					...
					if(currentApprovalLevel == 1) {
						...
						$scope.approverLevels[1].status = $scope.rateCardInfo[0].currentStatus;
					}
						...
						$scope.approverLevels[2].status = $scope.rateCardInfo[0].currentStatus;
					}
						...
						$scope.approverLevels[3].status = $scope.rateCardInfo[0].currentStatus;
					}
						...
						$scope.approverLevels[4].status = $scope.rateCardInfo[0].currentStatus;
					}
						...
						$scope.approverLevels[5].status = $scope.rateCardInfo[0].currentStatus;
					}
						...
						$scope.approverLevels[6].status = $scope.rateCardInfo[0].currentStatus;
					}
...
//					console.log("Inside the for loop");
		  ...
		  $scope.getApproverNames(rpVrsId);
	  }
	};
	...
	$scope.getApproverNames = function (rpVrsId) {
...
//  					console.log("Inside the for loop");
		 ...
		 WebServiceFactory.getApproverNames(rpVrsId).then(getApproverNames);
	}
